...
mixin GridMixin {
  ...
  /// 搜索结果的加载骨架，与 [searchResultList] 的布局保持一致
  Widget get searchSkeleton => Pref.searchSingleColumn
      ? const SliverList(
  ...
  /// 搜索结果列表：按设置决定使用单列（按行排列）或多列网格
  ///
  /// 当 [Pref.searchSingleColumn] 为 true 时返回 [SliverList]，
  /// 卡片撑满整行；否则回退到多列 [SliverGrid]。
  Widget searchResultList({
    required int itemCount,
    required IndexedWidgetBuilder itemBuilder,
  }) {
    if (Pref.searchSingleColumn) {
